jQuery(function(jQuery) {  
	...
	*/
	...
	jQuery('#sortable-medias .add_image').click(function() {
		...
		wp.media.editor.send.attachment = function(props, attachment){
		  if ( _custom_media ) {
			...
			jQuery( "#sortable-medias #sortable" ).append('<li class="ui-state-default" title="image"><img src="'+newimgurl+'" class="'+attachment.id+'"><div class="delete"><span></span></div></li>');
		  } else {
	...
	jQuery('#sortable-medias .add_video').click(function() {
		jQuery( "#sortable-medias #sortable" ).append('<li class="ui-state-default" title="video"><div class="move">move</div><textarea name="videovalue"></textarea><div class="delete"><span></span></div></li>');
	    mediaupdate();
		return false;  
	});
	...
	jQuery( "#sortable-medias #sortable").on("change", 'textarea', function() {
  	...
  	function mediaupdate() {
		...
		output = output.slice(0, -3);
		jQuery( "#sortable-medias #sortable" ).siblings('textarea').val(output);
	}
