...
if( !function_exists( 'sr_widgets_init' ) ) {
	function sr_widgets_init() {
		...
		register_sidebar( array(
			'name' => __( 'Sidebar Blog', 'sr_yalu_theme' ),
			'id' => 'sidebar-blog',
			'before_widget' => '<div class="widget clearfix">',
			'after_widget' => "</div>",
			'before_title' => '<h4 class="title"><span><strong>',
			'after_title' => '</strong></span></h4>'
		) );
		
		register_sidebar( array(
			'name' => __( 'Sidebar Page', 'sr_yalu_theme' ),
			'id' => 'sidebar-page',
			'before_widget' => '<div class="widget clearfix">',
			'after_widget' => "</div>",
			'before_title' => '<h4 class="title"><span><strong>',
		...
		register_sidebar( array(
			'name' => __( 'Footer Left', 'sr_yalu_theme' ),
			'id' => 'footer-left',
			'before_widget' => '<div class="widget clearfix">',
			'after_widget' => "</div>",
			'before_title' => '<h4 class="title"><strong>',
			'after_title' => '</strong></h4>'
		) );
		
		register_sidebar( array(
			'name' => __( 'Footer Center Left', 'sr_yalu_theme' ),
		...
		register_sidebar( array(
			'name' => __( 'Footer Center Right', 'sr_yalu_theme' ),
		...
		register_sidebar( array(
			'name' => __( 'Footer Right', 'sr_yalu_theme' ),
			'id' => 'footer-right',
			'before_widget' => '<div class="widget clearfix">',
			'after_widget' => "</div>",
			'before_title' => '<h4 class="title"><strong>',
...
if ( !function_exists( 'content' ) ) {
	function sr_password_form() {
		...
		<div class="form-row clearfix">
			<label for="comment_form" class="req">'.__( "To view this protected post, enter the password below:", "sr_yalu_theme" ).'</label>
			<div class="form-value"><input name="post_password" id="' . $label . '" type="password" size="20" /></div>
		</div>
		<div class="form-row clearfix"><input type="submit" name="Submit" value="' . __( "Submit", "sr_yalu_theme" ) . '" /></div>
		</form>
		';
		echo $o;
	}
...
if ( !function_exists( 'sr_getTitle' ) ) {
	function sr_getTitle() {
		...
		if (is_404()) {
			$sr_titles['tax'] = false;
			$sr_titles['title'] = __("Page not found", 'sr_yalu_theme');
		} else if (is_tag()) {
			$sr_titles['tax'] = single_tag_title('', false);
			$sr_titles['title'] = __("Tag", 'sr_yalu_theme');
		} else if (is_category()) {
			$sr_titles['tax'] = single_cat_title('', false);
			$sr_titles['title'] = __("Category", 'sr_yalu_theme');
		} else if (is_search()) {
			$sr_titles['tax'] = get_search_query();
			$sr_titles['title'] = __("Search", 'sr_yalu_theme');
			...
			$sr_titles['title'] = __("Portfolio", 'sr_yalu_theme');
			...
			$sr_titles['title'] = __("Archive", 'sr_yalu_theme');
		} else if (is_home()) {
			if (get_option('page_for_posts') > 0) {
				$blog = get_post(get_option('page_for_posts'));
				$title = $blog->post_title;
			} else {
				$title = __("Home", 'sr_yalu_theme');
			}
			$sr_titles['tax'] = false;
...
/* Breadcrumb
/*-----------------------------------------------------------------------------------*/
if ( !function_exists( 'sr_breadcrumb' ) ) {
	function sr_breadcrumb() {
	  ...
	  if (is_front_page()) {
		  ...
		  if ($theId) {
			  $pdata = get_post($theId);
			  $trail .= ' &raquo; <a   href="'.get_permalink($pdata->ID).'">'.$pdata->post_title.'</a>';
		  }
	  }
	  elseif (is_page()) {
	   $bcarray = array();
...
$sr_comments_defaults = array( 
	'comment_field' => '<div class="form-row clearfix textbox">
						<label for="comment_form" class="req">'.__('Comment', 'sr_yalu_theme').' *</label>
	...
	'title_reply'          => '',
	'title_reply_to'       => '',
	'cancel_reply_link'    => __( 'Click here to cancel reply', 'sr_yalu_theme' ),
	'label_submit'         => __( 'Post Comment', 'sr_yalu_theme' )
);
	...
	if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
		...
		switch($comment_status){
			...
			case '1': //Approved comment
				echo '<span class="confirm_icon">'.__('Confirm', 'sr_yalu_theme').'</span>
					  <span class="confirm_message"><strong>'.__( 'Your Comment has ben sent. Thank you.', 'sr_yalu_theme' ).'</strong></span>';
...
/*	Google Map  
/*-----------------------------------------------------------------------------------*/
function sr_googleMap($latlong, $text, $pin, $height, $id) {
	global $sr_prefix;
		...
		return '<div id="map'.$id.'" class="google-map" style="height:'.$height.'px;"></div>
				...
				jQuery(document).ready(function(){
					...
					if(jQuery().jPlayer) {
						jQuery("#jquery_jplayer_video'.$id.'").jPlayer({
							...
							swfPath: "http://spab-rice.com/wordpress/yalu/wp-content/themes/yalu/files/jplayer",
